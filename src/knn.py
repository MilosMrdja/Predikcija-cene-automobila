...
def get_max_acc(data,n):
    ...
    for col in columns:
        ...
        knn_regressor = KNeighborsRegressor(n_neighbors=n)  # isprobano vise vrdnosti za k
        knn_regressor.fit(x_train, y_train)
        acc_new = get_error_knn(knn_regressor, x_val, y_val)
        if acc_new > acc_best:
            acc_best = acc_new
...
def knn(data, n):
    ...
    dropp_to = []
    for i in range(2):
        knn_model, dropped_col, acc = get_max_acc(data,n)
    ...
    plt.title('Stvarne vs. Predviđene vrednosti KNN modela')
    plt.show()

    return model_best, acc_best

    
# radimo nad tes/val x i y
